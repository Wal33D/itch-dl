...
export async function loadKeysAndUrls(client: ItchApiClient): Promise<void> {
  ...
  let page = 1;
  while (true) {
    const res = await client.get('/profile/owned-keys', true, false, { params: { page }, timeout: 15000 });
    if (res.status !== 200) break;
    const data = res.data;
    if (!data.owned_keys) break;
...
export async function getOwnedKeys(client: ItchApiClient): Promise<[Record<number, string>, string[]]> {
  if (!KEYS_CACHED) await loadKeysAndUrls(client);
